<template>
  ...
  <div :class="{'open': isSort || isScreen}" @click.self="hideView">
    ...
    <section class="filter-extend" v-if="isSort">
      <ul>
        <li v-for="(item,index) in filterData.sortBy" :key="index" @click="selectSort(index)">
...
export default {
  ...
  data() {
    return {
      currentFilter: 0, //标题栏加粗控制符
      isSort: false, //蒙版控制符
      currentSort: 0, //下拉栏格式控制符
      isScreen: false //‘筛选’ 蒙版控制符
    };
  },
  props: {
    filterData: Object
  ...
  methods: {
    ...
    selectSort(index) {
      ...
      // console.log(this.filterData.sortBy[index].code);
      this.filterData.navTab[0].name = this.filterData.sortBy[index].name;
      this.hideView();
      //更新数据
      this.$emit("update", { condition: this.filterData.sortBy[index].code });
    },
    // 筛选栏点击
